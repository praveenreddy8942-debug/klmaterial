...
class ParticleSystem {
  ...
  createCanvas() {
    ...
    this.canvas.style.cssText = 'position: fixed; top: 0; left: 0; width: 100%; height: 100%; z-index: 0; pointer-events: none; opacity: 0.8;';
...
class ScrollReveal {
  ...
  observe() {
    ...
    const selectors = ['.card', '.about-section', '.contact-card-modern', '.stat-card', '.feature-item', '.year-card', '.resource-card', '.repeat-card', '.stat-item', '.category-btn', '.glass-card', '.stagger-item', '.profile-card', '.text-card', '.feature-card'];
...
class CustomCursor {
  ...
  attachEventListeners() {
    ...
    document.addEventListener('mouseover', (e) => {
      if (e.target.closest('a, button, .card, input, textarea, .material-card, .gchat-qr-btn, .selector-pill, .btn-primary, .btn-glass, .nav-link, .glass-card, .glass-input')) {
